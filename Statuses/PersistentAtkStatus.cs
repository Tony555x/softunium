...
public class PersistentAtkStatus : Status
{
    ...
    {
        if (newStatus is PersistentAtkStatus other)
        {
    ...
    public override string GetDescription()
    {
        ...
        return $"{action} Атаката с {(int)Math.Abs(total * 100)}% (Персистентно).";
    }
